...
def fixture_ansible_adhoc(request):  # type: ignore[no-untyped-def]
    """Return an inventory initialization method."""
    plugin = request.config.pluginmanager.getplugin("ansible")

    def init_host_mgr(**kwargs):  # type: ignore[no-untyped-def]
...
def fixture_ansible_module(ansible_adhoc):  # type: ignore[no-untyped-def]
...
@pytest.fixture()
def ansible_facts(ansible_module):  # type: ignore[no-untyped-def]
    """Return ansible_facts dictionary."""
    return ansible_module.setup()


@pytest.fixture()
def localhost(request):  # type: ignore[no-untyped-def]
